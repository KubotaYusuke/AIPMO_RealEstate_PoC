...
def ics_for_event(event_id, title, date_iso, description=""):
    ...
    lines = [
        ...
        fold_ics_line("SUMMARY", summary),
        fold_ics_line("DESCRIPTION", desc) if desc else "DESCRIPTION:",
        "BEGIN:VALARM",
        "TRIGGER:-P1D",
        "ACTION:DISPLAY",
...
def build_pack(row, lang):
    ...
    if pd.isna(date_dt):
        date_dt = pd.to_datetime(row.get("date"))
...
def load_kpi():
    ...
    def col_for(keys):
        ...
        return None
    c_date = col_for(KPI_COLS["date"])
    c_pv = col_for(KPI_COLS["pv"])
    c_inq = col_for(KPI_COLS["inq"])
    c_view = col_for(KPI_COLS["view"])
    c_offer = col_for(KPI_COLS["offer"])
    missing = [("date",c_date),("pv",c_pv),("inquiries",c_inq),("viewings",c_view),("offers",c_offer)]
    miss = [k for k,c in missing if c is None]
    if miss:
        return None, f"kpi.csv の列名が不足しています（必要: date, pv, inquiries, viewings, offers いずれかの表記）不足: {miss}"
    out = pd.DataFrame({
...
def kpi_aggregate(df, range_mode):
    today = pd.Timestamp(dt.date.today())
    if range_mode == "直近30日":
        start = today - pd.Timedelta(days=30)
        scope = df[df["date"] >= start]
    elif range_mode == "今月":
        ...
        scope = df.copy()
    if scope.empty:
        return "データがありません", "", pd.DataFrame(columns=df.columns)
    pv = scope["pv"].sum()
    inq = scope["inquiries"].sum()
    view = scope["viewings"].sum()
    off = scope["offers"].sum()
    def pct(a,b):
        return (a*100.0/b) if b>0 else 0.0
    resp = pct(inq,pv)
    conv_view = pct(view,inq)
    conv_offer = pct(off,view)
    days = (scope["date"].max() - scope["date"].min()).days + 1
    # cards HTML
    html = f"""
    <div style="display:flex; gap:12px; flex-wrap:wrap">
      <div style="flex:1; min-width:180px; padding:12px; border:1px solid #ddd; border-radius:10px">
        <div style="font-size:12px; color:#666">反響率</div>
        <div style="font-size:28px; font-weight:700">{resp:.1f}%</div>
        <div style="font-size:12px; color:#666">{inq} / {pv}</div>
      </div>
      <div style="flex:1; min-width:180px; padding:12px; border:1px solid #ddd; border-radius:10px">
        <div style="font-size:12px; color:#666">内覧化率</div>
        <div style="font-size:28px; font-weight:700">{conv_view:.1f}%</div>
        <div style="font-size:12px; color:#666">{view} / {inq}</div>
      </div>
      <div style="flex:1; min-width:180px; padding:12px; border:1px solid #ddd; border-radius:10px">
        <div style="font-size:12px; color:#666">申込率</div>
        <div style="font-size:28px; font-weight:700">{conv_offer:.1f}%</div>
        <div style="font-size:12px; color:#666">{off} / {view}</div>
      </div>
      <div style="flex:1; min-width:180px; padding:12px; border:1px solid #ddd; border-radius:10px">
        <div style="font-size:12px; color:#666">経過日数</div>
        <div style="font-size:28px; font-weight:700">{days} 日</div>
        <div style="font-size:12px; color:#666">{scope['date'].min().date()} → {scope['date'].max().date()}</div>
      </div>
    </div>
    """
    return "", html, scope
...
def init_kpi(range_mode):
    df, enc = load_kpi()
    if df is None:
        return f"読み込みエラー: {enc}", "", pd.DataFrame()
    msg, html, scope = kpi_aggregate(df, range_mode)
    return msg, html, scope

with gr.Blocks() as demo:
    gr.Markdown("## AI売却PMO（PoC） — PMO+（チェックリスト/文例/Slack/ICS + KPI）")
    with gr.Tabs():
        ...
        with gr.TabItem("KPI"):
            gr.Markdown("KPIミニダッシュボード（`data/kpi.csv` を読み込み）")
            range_mode = gr.Radio(["直近30日","今月","すべて"], value="直近30日", label="集計範囲")
            kpi_refresh = gr.Button("KPI更新")
            kpi_msg = gr.Textbox(label="メッセージ", lines=1)
            kpi_cards = gr.HTML()
            kpi_table = gr.Dataframe(label="集計対象の明細")
            kpi_refresh.click(init_kpi, inputs=range_mode, outputs=[kpi_msg, kpi_cards, kpi_table])
            demo.load(init_kpi, inputs=range_mode, outputs=[kpi_msg, kpi_cards, kpi_table])

    gr.Markdown("※ KPIは `data/kpi.csv`（列: date, pv, inquiries, viewings, offers）をUTF-8で保存してください。Excelのときは“CSV UTF-8”保存が安全。")
