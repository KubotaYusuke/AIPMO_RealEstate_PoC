...
def fold_ics_line(name: str, value: str) -> str:
    # RFC5545 line folding at 75 octets; we use ~73 chars for simplicity.
    raw = f"{name}:{value}"
    out = []
    while len(raw) > 73:
        out.append(raw[:73])
...
def build_pack(row, lang):
    # row is a pandas Series with date_dt
...
with gr.Blocks() as demo:
    gr.Markdown("## AI売却PMO（PoC） — PMO+（チェックリスト/文例/Slack/ICS, 予定メモ対応）")
    with gr.Row():
    ...
    gr.Markdown("※ 予定のメモ欄に概要を自動記入します（SUMMARYとDESCRIPTIONを設定、終日扱いでDTENDは翌日）。")
