...
def read_kpi():
    ...
    if not p.exists(): return None, "kpi.csv がありません（data/kpi.csv を作成してください）"
    df, enc = read_csv_flex(p)
    ...
    if miss: return None, f"kpi.csv 列不足: {miss}"
    out = pd.DataFrame({
...
def kpi_aggregate(range_mode):
    df, enc = read_kpi()
    if df is None: return f"読み込みエラー: {enc}", "", pd.DataFrame()
    ...
    if range_mode == "直近30日":
        ...
        scope = df.copy()
    if scope.empty: return "対象期間にデータがありません", "", scope
    return "", kpi_cards_html(scope), scope
...
def retrieve_support(event_row, topk=3):
    chunks = load_rag()
    if not chunks: return "（根拠データが未設定です。`data/rag_chunks.jsonl` を用意するとここに要点が並びます）"
    ...
    for c in chunks:
        ...
        if score>0: scores.append((score, c))
    if not scores: return "（該当する根拠は見つかりませんでした）"
    scores.sort(key=lambda x: x[0], reverse=True)
    lines = []
    for s, c in scores[:topk]:
        src = c.get("source","note"); page = c.get("page",""); tag = c.get("tag","")
...
def generate_pack(mode, lang, selector, show_support):
    ...
    if top.empty: return "該当なし。", None, None, "（根拠データがありません）"
    try:
...
with gr.Blocks() as demo:
    gr.Markdown("## AI売却PMO（PoC） — Calm KPI + 根拠（RAG）※任意表示")
    with gr.Tabs():
        with gr.TabItem("アクション"):
            ...
            with gr.Accordion("根拠（任意・静かな表示）", open=False):
        ...
        with gr.TabItem("KPI"):
            ...
            kpi_refresh.click(lambda r: kpi_aggregate(r), inputs=range_mode, outputs=[kpi_msg, kpi_cards, kpi_table])
            demo.load(lambda r: kpi_aggregate(r), inputs=range_mode, outputs=[kpi_msg, kpi_cards, kpi_table])

    gr.Markdown("※ `data/rag_chunks.jsonl`（1行1JSON）を置くと、アクション選択時に落ち着いた“根拠”抜粋を表示します。")
